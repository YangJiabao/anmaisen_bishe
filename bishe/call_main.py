...
class MyMainForm(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.pushButton_10.clicked.connect(self.inquire_db)                      #  绑定查询数据库（按照品牌查询按钮）信号槽
        ...
        self.pushButton_12.clicked.connect(self.create_table)                    #  绑定开始-创建表
        # self.pushButton_13.clicked.connect(self.create_database)               #  绑定开始-创建库
        self.pushButton_11.clicked.connect(self.del_db)                          #  绑定删除数据信号槽
        self.pushButton_8.clicked.connect(lambda:self.user(1))                  #  绑定增加用户
        self.pushButton_9.clicked.connect(lambda:self.user(2))                  #  绑定删除用户   多个信号链接同一个槽需要判断信号来源
    ...
    def connect_db(self):
        ...
        try:
            ...
            self.cur.execute("select user from mysql.user") #查询当前数据库所有用户
            # info = cur.fetchall()
            b = self.cur.fetchall()
            exclude_list = ["root", "mysql.sys", "mysql.session", "mysql.infoschema"]   #  排除自带用户与root用户
            for i in b:
                if i[0] not in exclude_list:
                    self.textBrowser.append("数据库全部用户："+ i[0])  #  self.textBrowser.append(self.sender() + ' 是发送者')
        except:
            flag = 0
    ...
    def user(self, index):
        try:
            if self.lineEdit_4.text() != '':
                if index == 1:
                    sql = "create user {}@'%' identified by ''".format(self.lineEdit_4.text())
                    self.cur.execute(sql)
                    self.textBrowser.append("OAO！添加成功~~~")
                    self.cur.execute("select user from mysql.user")  # 查询当前数据库所有用户
                    # info = cur.fetchall()
                    b = self.cur.fetchall()
                    exclude_list = ["root", "mysql.sys", "mysql.session", "mysql.infoschema"]    #  排除自带用户与root用户
                    for i in b:
                        if i[0] not in exclude_list:
                            self.textBrowser.append("数据库全部用户：" + i[0])
                else:
                    sql = "drop user {}".format(self.lineEdit_4.text())
                    self.cur.execute(sql)
                    self.textBrowser.append("OAO！删除成功~~~")
                    self.cur.execute("select user from mysql.user")  # 查询当前数据库所有用户
                    # info = cur.fetchall()
                    b = self.cur.fetchall()
                    exclude_list = ["root", "mysql.sys", "mysql.session", "mysql.infoschema"]     #  排除自带用户与root用户
                ...
                self.textBrowser.append("QAQ！输入栏不能为空哦~~~")
        except:
            self.textBrowser.append("QAQ！请检查一下数据库连接哦~~~")
    # def create_database(self):
    #     #建库
    #     try:
    #         try:
    #             self.db.ping(reconnect=True)  # ping数据库是否关闭，关闭就打开
    #             self.cur.execute("CREATE DATABASE IF NOT EXISTS `%s` DEFAULT CHARSET utf8 COLLATE utf8_general_ci;") % (self.lineEdit_8.text()) #如果数据库不存在则创建，存在则不创建。创建RUNOOB数据库，并设定编码集为utf8
    ...
    def create_table(self):
        ...
        try:
            self.db.ping(reconnect=True)  # ping数据库是否关闭，关闭就打开
            if self.lineEdit_4.text() == '':
                self.textBrowser.append("QAQ！先填入表名哦~")
            else:
                sql = """
        ...
        if flag == 0 :
            self.pushButton_12.clicked.connect(self.create_table)
    ...
    def inquire_db(self):
        ...
        try:
            self.db.ping(reconnect=True)
            sql = "SELECT * FROM test_an1 WHERE 商品编号 = '%s'" % self.lineEdit_3.text()
            if self.lineEdit_3.text() == '':
                sql = "SELECT * FROM test_an1 WHERE 品牌 = '%s'" % self.lineEdit_6.text()
            elif self.lineEdit_6.text() != '':
                # sql = "SELECT * FROM test_an1 WHERE 商品编号 = '%s'" % self.lineEdit_3.text()
                self.textBrowser.append("只能输入编号或者品牌其中的一个哦~~~~")
            self.cur.execute(sql)
            info = self.cur.fetchall()
            # print(self.info)
            if info != ():
                for d in info:
                    self.textBrowser.append("编号为：" + list(d)[1] + "  品牌为：" + list(d)[6] + "  数量为：" + list(d)[9])
                    self.number = list(d)[1]
                    # print("编号为：" + list(d)[1])
                    # print("数量为：" + list(d)[9])  # 当前数量
            else:
                self.textBrowser.append("没找到哦~~~输入是正确的吗？")
    ...
    def del_db(self):
        try:
            self.db.ping(reconnect=True)
            sql_delete = "DELETE FROM test_an1 WHERE 商品编号 = '%s'"  % self.number
            self.cur.execute(sql_delete)
            self.db.commit()
            self.textBrowser.append("编号为：" + self.number +" 数据已删除。")
        except:
            self.textBrowser.append("QAQ！先查询一下数据哦~~~")
